	...
	ajaxForms.push({
		...
		init: function ()
		{
			$('.lk-step-btn').click(function(e){
				e.preventDefault();

				if ($('#formChequeReg input[name=upload]').val() == "")
					return false;

				$('#formChequeReg .lk-step-1').addClass('d-n');
				$('#formChequeReg .lk-step-2').removeClass('d-n');
				resize.refresh();
			});
		},
		beforeSubmit: function (arr, $form, options)
		{
			...
			if ($form.find('input[name=upload]').val() == "")
			{
		...
		success: function (responseText, statusText, xhr, $form)
		{
			dataLayer.push({'event': 'CheckRegOk'});
	...
	ajaxForms.push({
		id: 'formDataMatrixReg',
		init: function () 
		{
			$("#formDataMatrixReg input[name=upload]").fileinput({
				uploadUrl: '#',       
				language: "ru",         
				showUpload: false,
				showCaption: false,
